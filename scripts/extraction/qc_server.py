...
DETECTF = "qc_detect.json"   # vysledky "Identifikovat" (prezije reload)

def load_detect():
    try:
        with open(DETECTF, encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}

def save_detect(d):
    with open(DETECTF, "w", encoding="utf-8") as f:
...
def apply_action(action, payload):
    ...
    if action == "detect_save":
        pid = payload.get("id")
        if not pid:
            return False, "chybi id"
        det = load_detect()
        det[pid] = {"decision": payload.get("decision"), "finalName": payload.get("finalName"), "osm": bool(payload.get("osm"))}
        save_detect(det)
        return True, ""
...
def render():
    ...
    detect_json = json.dumps(load_detect(), ensure_ascii=False)
    ...
    list_html = (
        ...
        '<button onclick="runIdentify()" id="idBtn">\U0001F50D Identifikovat</button>'
        '<button onclick="stopIdentify()" id="idStop" style="display:none">\u23f9 Stop</button>'
    ...
    shared_js = ('var PLACES=' + map_places + ';var CATLABELS=' + cat_labels_json + ';var DETECT=' + detect_json + ';' + r'''
...
function rowHtml(p){var col=CATCOLOR[p.cat]||"#555";return '<div class="lrow" data-id="'+p.id+'"><div class="lhead"><input type="checkbox" class="lchk" data-id="'+p.id+'" style="margin-right:4px;flex:none"><span class="ldot" style="background:'+col+'"></span><span class="lname">'+escH(p.name)+'</span><span class="lcat">'+(CATLABELS[p.cat]||p.cat)+'</span>'+tierBadge(p.tier)+'<span class="lgps">'+p.lat.toFixed(5)+", "+p.lon.toFixed(5)+'</span></div>'+detChip(p)+'<div class="ledit" data-edit="'+p.id+'"></div></div>';}
function editHtml(p){var opts=Object.keys(CATLABELS).map(function(k){return '<option value="'+k+'"'+((k===p.cat)?" selected":"")+'>'+CATLABELS[k]+'</option>';}).join("");return '<div class="erow"><label>N\u00e1zev</label><input id="e-name-'+p.id+'" value="'+escH(p.name).replace(/"/g,"&quot;")+'"></div><div class="erow"><label>Kategorie</label><select id="e-cat-'+p.id+'">'+opts+'</select></div><div class="erow"><label>GPS</label><input id="e-lat-'+p.id+'" value="'+p.lat+'" style="width:120px"> <input id="e-lon-'+p.id+'" value="'+p.lon+'" style="width:120px"></div><div id="mini-'+p.id+'" class="mini"></div><p class="meta">T\u00e1hni \u0161pend\u00edk pro posun (p\u0159ep\u00ed\u0161e GPS pole). Po \u00faprav\u011b dej Ulo\u017eit.</p><div class="ebtns"><button class="ok" data-lact="save" data-id="'+p.id+'">\U0001F4BE Ulo\u017eit</button><button data-lact="verify" data-id="'+p.id+'">\u2713 Ov\u011b\u0159it (tier A)</button><button class="del" data-lact="delete" data-id="'+p.id+'">Smazat</button><button data-lact="detect" data-id="'+p.id+'">\U0001F50D Test rozpozn\u00e1n\u00ed</button><button data-lact="osmmatch" data-id="'+p.id+'">\U0001F5FA Z mapy</button></div><div id="detect-'+p.id+'" class="detectbox"></div>';}
function detInner(p,d){var DC={auto_save:["#1a7f37","auto"],suggest:["#b8860b","n\u00e1vrh"],reject:["#999","zahozeno"],error:["#d33","chyba"]};var c=DC[d.decision]||["#999",d.decision||"?"];var fn=d.finalName||"\u2014";var match="";if(d.osm){match=' <span style="color:#1a7f37" title="p\u0159evzato z mapy (OSM) dle n\u00e1zvu">\ud83d\uddfa</span>';}else if(d.finalName){var ok=nameSim(p.name,d.finalName)>=0.6;match=ok?' <span style="color:#1a7f37">\u2713</span>':' <span style="color:#d33" title="li\u0161\u00ed se od tv\u00e9ho n\u00e1zvu">\u2717</span>';}return '<span style="color:'+c[0]+';font-weight:600">'+c[1]+'</span> \u2192 '+(d.osm?'\ud83d\uddfa ':'')+'"'+escH(fn)+'"'+match;}
function detChip(p){return '<div class="ldet" id="ldet-'+p.id+'" style="font-size:12px;padding:0 10px 6px 26px;color:#555">'+(DETECT[p.id]?detInner(p,DETECT[p.id]):'')+'</div>';}
function updateRowChip(id){var el=document.getElementById("ldet-"+id);if(!el)return;var p=PLACES.find(function(x){return x.id===id;});el.innerHTML=DETECT[id]?detInner(p,DETECT[id]):'';}
function qpost(action,payload){fetch("/api/"+action,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).catch(function(){});}
var identifyRunning=false;var identifyStop=false;
function selectedIds(){var c=[].slice.call(document.querySelectorAll("#listContainer .lchk:checked")).map(function(x){return x.getAttribute("data-id");});if(c.length)return c;return [].slice.call(document.querySelectorAll("#listContainer .lrow")).map(function(r){return r.getAttribute("data-id");});}
function toggleAllChk(){var all=[].slice.call(document.querySelectorAll("#listContainer .lchk"));var anyOff=all.some(function(x){return !x.checked;});all.forEach(function(x){x.checked=anyOff;});}
function stopIdentify(){identifyStop=true;}
var osmCands={};
function nameSim(a,b){function nrm(s){return (""+(s||"")).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9 ]/g," ").replace(/\s+/g," ").trim();}var x=nrm(a),y=nrm(b);if(!x||!y)return 0;if(x===y)return 1;var contain=(x.length>=3&&y.indexOf(x)>=0)||(y.length>=3&&x.indexOf(y)>=0);function bg(s){var m={};for(var i=0;i<s.length-1;i++){var g=s.substr(i,2);m[g]=(m[g]||0)+1;}return m;}var bx=bg(x),by=bg(y),inter=0,total=0,g;for(g in bx){total+=bx[g];if(by[g])inter+=Math.min(bx[g],by[g]);}for(g in by)total+=by[g];var dice=total?(2*inter)/total:0;var tx=x.split(" ").filter(function(w){return w.length>=3;}),ty=y.split(" ").filter(function(w){return w.length>=3;});var shared=0;tx.forEach(function(w){if(ty.indexOf(w)>=0)shared++;});var tok=(tx.length&&ty.length)?shared/Math.min(tx.length,ty.length):0;var sim=Math.max(dice,tok);if(contain)sim=Math.max(sim,0.65);return Math.round(sim*100)/100;}
function storeDet(id,decision,finalName,osm){DETECT[id]={decision:decision,finalName:finalName||null,osm:!!osm};qpost("detect_save",{id:id,decision:decision,finalName:finalName||null,osm:!!osm});updateRowChip(id);}
...
function runIdentify(){if(identifyRunning)return;var ids=selectedIds();if(!ids.length)return;identifyRunning=true;identifyStop=false;var prog=document.getElementById("idProg");var btn=document.getElementById("idBtn");var stop=document.getElementById("idStop");if(btn)btn.disabled=true;if(stop)stop.style.display="";var i=0;function fin(msg){identifyRunning=false;if(btn)btn.disabled=false;if(stop)stop.style.display="none";if(prog)prog.textContent=msg;}function step(){if(identifyStop){fin("zastaveno ("+i+"/"+ids.length+")");return;}if(i>=ids.length){fin("hotovo ("+ids.length+")");return;}var id=ids[i];var p=PLACES.find(function(x){return x.id===id;});if(prog)prog.textContent="identifikuji "+(i+1)+"/"+ids.length+"\u2026";processOne(id,p).then(function(){i++;setTimeout(step,400);});}step();}
...
function detectRow(id){var lat=parseFloat(document.getElementById("e-lat-"+id).value);var lon=parseFloat(document.getElementById("e-lon-"+id).value);var box=document.getElementById("detect-"+id);if(!box)return;box.innerHTML='<p class="meta">Rozpozn\u00e1v\u00e1m...</p>';var url="http://"+location.hostname+":3000/detect/preview?lat="+lat+"&lon="+lon+"&gap=15";fetch(url).then(function(r){return r.json();}).then(function(d){if(d.error){box.innerHTML='<p style="color:#d33">Chyba: '+escH(d.error)+'</p>';return;}var DC={auto_save:["#1a7f37","AUTO-ULO\u017dENO"],suggest:["#b8860b","N\u00c1VRH (?)"],reject:["#999","ZAHOZENO"]};var dc=DC[d.decision]||["#999",d.decision];var h='<div style="border-top:1px solid #ddd;margin-top:8px;padding-top:8px;font-size:13px">';h+='<div><b style="color:'+dc[0]+'">'+dc[1]+'</b>'+(d.finalName?' \u2192 "'+escH(d.finalName)+'"':'')+'</div>';if(d.map&&d.map.name){h+='<p class="meta">\ud83d\uddfa mapa: '+escH(d.map.name)+" ("+escH(d.map.kind)+")</p>";}if(d.geocode){h+='<p class="meta">adresa: '+escH(d.geocode.formatted||"-")+(d.residential?" \u00b7 REZIDEN\u010cN\u00cd":"")+'</p>';}if(d.atAddress){h+='<p class="meta">na adrese: '+escH(d.atAddress.name)+" (shoda "+(d.atAddress.addrScore||0)+")</p>";}if(d.osm){h+='<p class="meta">OSM: '+escH(d.osm.name)+" ("+escH(d.osm.kind)+" \\u00b7 "+d.osm.dist+"m)</p>";}if(d.ai){h+='<p class="meta">AI: "'+escH(d.ai.name||"null")+'" conf='+d.ai.confidence+" \u2014 "+escH(d.ai.reason||"")+'</p>';}h+='<p class="meta">n\u00e1v\u0161t\u011bv: '+d.historyVisits+" \u00b7 v\u00fdb\u011br dle adresy: "+(d.addrPick?escH(d.addrPick):"-")+'</p>';if(d.nearby&&d.nearby.length){h+='<p class="meta" style="margin-top:4px"><b>POI v okol\u00ed:</b></p>';d.nearby.slice(0,8).forEach(function(p){h+='<p class="meta">\u2022 '+escH(p.name)+" \u2014 "+(p.primaryType||"?")+" \u00b7 "+p.dist+"m"+(p.addrScore?" \u00b7 adr"+p.addrScore:"")+'</p>';});}h+='</div>';box.innerHTML=h;DETECT[id]={decision:d.decision,finalName:d.finalName||null};qpost("detect_save",{id:id,decision:d.decision,finalName:d.finalName||null});updateRowChip(id);}).catch(function(e){box.innerHTML='<p style="color:#d33">Nedostupn\u00e9 (b\u011b\u017e\u00ed Node na :3000?): '+escH(""+e)+'</p>';});}
function osmMatchRow(id){var nm=document.getElementById("e-name-"+id).value;var lat=parseFloat(document.getElementById("e-lat-"+id).value);var lon=parseFloat(document.getElementById("e-lon-"+id).value);var box=document.getElementById("detect-"+id);if(!box)return;box.innerHTML='<p class="meta">Hled\u00e1m v map\u011b (OSM)\u2026</p>';var url="http://"+location.hostname+":3000/osm/match?lat="+lat+"&lon="+lon+"&radius=150&name="+encodeURIComponent(nm);fetch(url).then(function(r){return r.json();}).then(function(m){if(m.error){box.innerHTML='<p style="color:#d33">Chyba: '+escH(m.error)+'</p>';return;}var cs=m.candidates||[];osmCands[id]=cs;if(!cs.length){box.innerHTML='<p class="meta">\u017d\u00e1dn\u00e9 pojmenovan\u00e9 OSM objekty v okol\u00ed (150m)</p>';return;}var h='<div style="border-top:1px solid #ddd;margin-top:8px;padding-top:8px;font-size:13px"><p class="meta">OSM objekty dle podobnosti k \"'+escH(nm)+'\" \u2014 klikni pro p\u0159evzet\u00ed do n\u00e1zvu:</p>';cs.forEach(function(c,idx){var pct=Math.round((c.sim||0)*100);var col=pct>=50?"#1a7f37":(pct>=25?"#b8860b":"#999");h+='<p class="meta osmpick" data-id="'+id+'" data-idx="'+idx+'" style="cursor:pointer;padding:2px 0"><b style="color:'+col+'">'+pct+'%</b> \u2022 '+escH(c.name)+' <span style="color:#999">('+escH(c.kind)+" \u00b7 "+c.dist+'m)</span></p>';});h+='</div>';box.innerHTML=h;}).catch(function(e){box.innerHTML='<p style="color:#d33">Nedostupn\u00e9 (b\u011b\u017e\u00ed Node na :3000?): '+escH(""+e)+'</p>';});}
document.getElementById("listContainer").addEventListener("click",function(e){if(e.target.classList&&e.target.classList.contains("lchk"))return;var pk=e.target.closest(".osmpick");if(pk){var pid=pk.getAttribute("data-id"),pix=+pk.getAttribute("data-idx");var pc=(osmCands[pid]||[])[pix];if(pc){var nf=document.getElementById("e-name-"+pid);if(nf)nf.value=pc.name;}return;}var b=e.target.closest("[data-lact]");if(b){var a=b.getAttribute("data-lact"),id=b.getAttribute("data-id");if(a==="save")saveRow(id);else if(a==="verify")verifyRow(id);else if(a==="delete")deleteRow(id);else if(a==="detect")detectRow(id);else if(a==="osmmatch")osmMatchRow(id);return;}var head=e.target.closest(".lhead");if(head){var row=head.closest(".lrow");var id=row.getAttribute("data-id");if(expandedId===id)closeRow();else openRow(id);}});
