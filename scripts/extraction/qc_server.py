...
function editHtml(p){var opts=Object.keys(CATLABELS).map(function(k){return '<option value="'+k+'"'+((k===p.cat)?" selected":"")+'>'+CATLABELS[k]+'</option>';}).join("");return '<div class="erow"><label>N\u00e1zev</label><input id="e-name-'+p.id+'" value="'+escH(p.name).replace(/"/g,"&quot;")+'"></div><div class="erow"><label>Kategorie</label><select id="e-cat-'+p.id+'">'+opts+'</select></div><div class="erow"><label>GPS</label><input id="e-lat-'+p.id+'" value="'+p.lat+'" style="width:120px"> <input id="e-lon-'+p.id+'" value="'+p.lon+'" style="width:120px"></div><div id="mini-'+p.id+'" class="mini"></div><p class="meta">T\u00e1hni \u0161pend\u00edk pro posun (p\u0159ep\u00ed\u0161e GPS pole). Po \u00faprav\u011b dej Ulo\u017eit.</p><div class="ebtns"><button class="ok" data-lact="save" data-id="'+p.id+'">\ud83d\udcbe Ulo\u017eit</button><button data-lact="verify" data-id="'+p.id+'">\u2713 Ov\u011b\u0159it (tier A)</button><button class="del" data-lact="delete" data-id="'+p.id+'">Smazat</button><button data-lact="detect" data-id="'+p.id+'">\ud83d\udd0d Test rozpozn\u00e1n\u00ed</button><button data-lact="osmmatch" data-id="'+p.id+'">\ud83d\uddfa Z mapy</button></div><div id="detect-'+p.id+'" class="detectbox"></div>';}
