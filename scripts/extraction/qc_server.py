...
def render():
    ...
    list_html = (
        ...
        '<button onclick="identifyVisible()" id="idBtn">\U0001F50D Identifikovat viditeln\u00e9</button>'
...
var identifyRunning=false;
function identifyVisible(){if(identifyRunning)return;var ids=[].slice.call(document.querySelectorAll("#listContainer .lrow")).map(function(r){return r.getAttribute("data-id");});if(!ids.length)return;identifyRunning=true;var prog=document.getElementById("idProg");var btn=document.getElementById("idBtn");if(btn)btn.disabled=true;var i=0;function step(){if(i>=ids.length){identifyRunning=false;if(btn)btn.disabled=false;if(prog)prog.textContent="hotovo ("+ids.length+")";return;}var id=ids[i];var p=PLACES.find(function(x){return x.id===id;});if(prog)prog.textContent="identifikuji "+(i+1)+"/"+ids.length+"\u2026";var url="http://"+location.hostname+":3000/detect/preview?lat="+p.lat+"&lon="+p.lon+"&gap=15";fetch(url).then(function(r){return r.json();}).then(function(d){DETECT[id]={decision:d.decision,finalName:d.finalName||null};qpost("detect_save",{id:id,decision:d.decision,finalName:d.finalName||null});updateRowChip(id);}).catch(function(){DETECT[id]={decision:"error",finalName:null};updateRowChip(id);}).then(function(){i++;setTimeout(step,400);});}step();}
