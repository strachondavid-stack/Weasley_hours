...
def apply_action(action, payload):
    ...
    if action == "detect_save":
        ...
        det = load_detect()
        det[pid] = {"decision": payload.get("decision"), "finalName": payload.get("finalName")}
        save_detect(det)
        return True, ""
...
function detInner(p,d){var DC={auto_save:["#1a7f37","auto"],suggest:["#b8860b","n\u00e1vrh"],reject:["#999","zahozeno"],error:["#d33","chyba"]};var c=DC[d.decision]||["#999",d.decision||"?"];var fn=d.finalName||"\u2014";var match="";if(d.finalName){var a=(""+p.name).toLowerCase().trim(),b=(""+d.finalName).toLowerCase().trim();match=(a===b)?' <span style="color:#1a7f37">\u2713</span>':' <span style="color:#d33" title="li\u0161\u00ed se od tv\u00e9ho n\u00e1zvu">\u2717</span>';}return '<span style="color:'+c[0]+';font-weight:600">'+c[1]+'</span> \u2192 "'+escH(fn)+'"'+match;}
...
function runIdentify(){if(identifyRunning)return;var ids=selectedIds();if(!ids.length)return;identifyRunning=true;identifyStop=false;var prog=document.getElementById("idProg");var btn=document.getElementById("idBtn");var stop=document.getElementById("idStop");if(btn)btn.disabled=true;if(stop)stop.style.display="";var i=0;function fin(msg){identifyRunning=false;if(btn)btn.disabled=false;if(stop)stop.style.display="none";if(prog)prog.textContent=msg;}function step(){if(identifyStop){fin("zastaveno ("+i+"/"+ids.length+")");return;}if(i>=ids.length){fin("hotovo ("+ids.length+")");return;}var id=ids[i];var p=PLACES.find(function(x){return x.id===id;});if(prog)prog.textContent="identifikuji "+(i+1)+"/"+ids.length+"\u2026";var url="http://"+location.hostname+":3000/detect/preview?lat="+p.lat+"&lon="+p.lon+"&gap=15";fetch(url).then(function(r){return r.json();}).then(function(d){DETECT[id]={decision:d.decision,finalName:d.finalName||null};qpost("detect_save",{id:id,decision:d.decision,finalName:d.finalName||null});updateRowChip(id);}).catch(function(){DETECT[id]={decision:"error",finalName:null};updateRowChip(id);}).then(function(){i++;setTimeout(step,400);});}step();}
